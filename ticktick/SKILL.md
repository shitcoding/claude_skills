...
Configuration is split between a JSON file (metadata) and `pass` (credentials):

**JSON config** (`~/.clawdbot/credentials/ticktick-cli/config.json`) - metadata only:
```json
{
...
**Credentials in pass** (GPG-encrypted):
```
ticktick-cli/client-id
ticktick-cli/client-secret
ticktick-cli/access-token
ticktick-cli/refresh-token
```
...
3. Before first auth, store your client credentials:
   ```bash
   pass insert ticktick-cli/client-id
   pass insert ticktick-cli/client-secret
   ```

The CLI automatically refreshes tokens when they expire and stores them securely in pass.
